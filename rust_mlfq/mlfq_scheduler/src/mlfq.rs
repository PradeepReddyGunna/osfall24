...
pub struct MLFQ {
    pub queues: Vec<Vec<Process>>,  // Make queues public
    pub num_levels: usize,           // Make num_levels public
    time_quanta: Vec<u32>,
    current_time: u32,
}
...
impl MLFQ {
    ...
    pub fn add_process(&mut self, process: Process) {
        ...
        if priority < self.num_levels {
            ...
            self.queues[self.num_levels - 1].push(process); // Place in lowest priority queue
        }
    }
    ...
    pub fn execute_process(&mut self, queue_index: usize) {
        if let Some(mut process) = self.queues[queue_index].pop() {
            ...
            println!("Executed process ID: {}, Executed Time: {}, Remaining Time: {}", 
            ...
            // If process is completed, it won't be added back to any queue
        }
    }
